...
На сервере и проверено запросами снаружи:
  ...
  страницы       index.html, 404.html, privacy.html, sitemap.xml,
                 en/index.html, en/privacy.html — совпадают с локальными
                 по sha256;
  ...
  IndexNow       все четыре адреса отправлены 17.09.2026, ответ 200
                 (в первый заход ушли только русские — исправлено).
...
Репозиторий и сервер совпадают: `index.html` и `en/index.html` с промо-блоком
TumioAI (`.tumio-promo`) забраны с сервера в `site/` (до этого блок жил только
на сервере и заливка папки затёрла бы его). Если править страницы прямо на
сервере, файлы надо после этого забирать обратно в репозиторий — иначе
следующая выкладка `site/` поверх всё равно потеряет правку.
...
Ключ уже лежит в корне (b3c2382811680ec6fe3687077f92b2f8.txt). **Все четыре
адреса отправлены 17.09.2026, ответ 200** — повторять сейчас не нужно.
Первый раз уходили только две русские страницы, это было упущение: в sitemap
английские есть, а в запросе их не было. Отправлять надо всё, что есть в
sitemap. Команда ниже — на следующее обновление сайта: Яндекс и Bing узнают
о правках сразу, не дожидаясь обхода.
  ...
  curl -X POST https://api.indexnow.org/indexnow \
    ...
    -d '{"host":"stickio.tumioai.ru",
         ...
         "urlList":["https://stickio.tumioai.ru/",
                    ...
                    "https://stickio.tumioai.ru/en/",
                    "https://stickio.tumioai.ru/en/privacy.html"]}'
